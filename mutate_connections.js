...
function replaceAfter(haystack, needle, pin, minIndex)
{
	...
	if (needle.toUpperCase()==needle && pin!=null)
		pin = pin.toUpperCase();
...
function parseConnection(text, connectionID, callback)
{
	...
	for (var senti = parsed.sentences.length - 1; senti >= 0; senti--) {
		...
		var ALL_CAPS_MODE = (sentence.text()==sentence.text().toUpperCase());
		...
		sentenceText = sentenceText.replace(/(you)\'re/gi,"$1 are");
		sentenceText = sentenceText.replace(/[Iu]\'ll/ig, " will");
		sentenceText = sentenceText.replace(/so i know it\'s you/gi," ");
		sentenceText = sentenceText.replace(/(I)\'ve/gi,"$1 has");
		sentenceText = sentenceText.replace(/(you)\'ve/gi,"$1 have");
		sentenceText = sentenceText.replace(/(I)\'m/gi,"$1 is");
		...
		if (/missed connections/gi.test(sentenceText)
			...
			||
			/(pussy| tits|fuck|shit|slut|bitch|nigger|fag)/gi.test(sentenceText)
			||
		...
		ritatags = RiTa.RiTa.getPosTags(mangled);

		for (var tokeni = 0; tokeni < sentence.tokens.length; tokeni++) 
		{
			token_tag = ritatags[tokeni].toUpperCase();
			next_token_tag = ritatags[tokeni + 1];
			...
			if (sentence.tokens.length > tokeni + 2 && (next_token.text==" " || next_token.text==""))
			{
				...
				next_token_tag = ritatags[tokeni + 2];
			}

			token_index = mangled.indexOf(token.text, token_index);
			if (verbose) console.log("Token: " + token.text + " (" + token_index + "," + token.pos.tag + "/" + token_tag +")");

			if (sentence.tokens.length > tokeni + 1 && next_token_tag=="VBD" && removeIfPastTenseAfter.indexOf(token.text.toLowerCase())>-1)
			{
			...
			else if (last_token_pos!="IN"  && (token_text.toLowerCase()=="i" || token_text.toLowerCase()=="it" || token_tag=="NN"))
			{
				last_subject = "it";
			...
			*/
			...
			)
			{
				...
				if (useInfAfter.indexOf(last_token.toLowerCase())==-1  &&
				...
				else if (last_subject=="you" || last_subject=="we" || useInfAfter.indexOf(last_token.toLowerCase())>-1)
				{
